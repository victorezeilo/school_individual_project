...
if($success){
	...
	$lastimid = isset($_POST['txt_lastimid']) ? filter_var($_POST['txt_lastimid'], FILTER_SANITIZE_NUMBER_INT):0;
	$message = isset($_POST["txt_message"]) ? filter_var($_POST["txt_message"], FILTER_SANITIZE_FULL_SPECIAL_CHARS):'';
	...
	elseif(empty($message)) {
		$success = false;
		$status_type = 'error';
		$status_msgs[] = 'Error: Invalid message text';
	}
...
if($success){
	...
	$im = new IM($data);
	$success = $im->save(true);
	$status_type = 'error';
	$status_msgs[] = 'Error: Failed to send message';
}
...
if($success){
	...
	//$db->execQuery("UPDATE tbl_im SET fStatus=21 WHERE fStatus=20 AND fTo=$userid AND fFrom=$contact_id");
	$html = '';
