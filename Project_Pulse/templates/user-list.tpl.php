						...
						<form name="form1" id="form1" method="post" action="include/submit.php">
							...
							<fieldset id="gridOptions">
								<ul>
									...
									<li><button type="button" class="btn-alt" onClick="location.href='user-import.php'"><i class="fa-solid fa-file-arrow-down mr-10"></i>Import Users</button></li>
									<li>
										<div>
											<label>
												<span>Sort:</span>
								...
								<ul>
									<li>
										...
										<label><input type="checkbox" id="checkAll" data-sync="true" class="mr-10">Select All</label>
										<label>
...
<script>
	jQuery(($) => {
		...
		$('button[name="btn_update"]').click(function(e){
			...
			const options = action === 'sendlink' ? 
						{message : `Send activation link to selected account(s)?`} :
						{message : `Are you sure you want to '${action}' selected account(s)?`};
							
			showDialog(options).then(function(response){
				if(response){
					$('input[name="btn_submit"]').val(action === 'sendlink' ? action : 'update');
					$('#form1').submit();	
				}
			});
